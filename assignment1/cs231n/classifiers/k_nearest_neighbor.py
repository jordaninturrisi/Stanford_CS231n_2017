...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    # Combine all elements and square root
    dists = np.sqrt(X_squared[:, np.newaxis] - 2*XY + Y_squared)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      # Use indices to determine k closest labels
      ...
      # Find most occurring index
      diff_labels = np.bincount(closest_y)
      y_pred[i] = np.argmax(diff_labels)
