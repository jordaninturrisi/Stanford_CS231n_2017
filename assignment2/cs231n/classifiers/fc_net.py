...
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_param):
    """
    Layer that performs affine - batch_norm - ReLU

    Inputs:
    - x: Input to the affine layer
    - w, b: Weights for the affine layer
    - gamma, beta, bn_param: parameters for batch norm
...
def affine_bn_relu_backward(dout, cache):
    """
    Backward pass for the affine - batch_norm - ReLU layer
    """
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ############################################################################
        # Initialise weights to Gaussian with mean=0 and std.dev=weight_scale
    ...
    def loss(self, X, y=None):
        ...
        # Include L2 regularisation with loss
        loss += 0.5 * self.reg * np.sum(self.params['W1'] * self.params['W1'])
        loss += 0.5 * self.reg * np.sum(self.params['W2'] * self.params['W2'])
        
        # Calculate derivatives for second layer input, weight, bias
        dx2, dw2, db2 = affine_backward(dscores, h2_cache)
        grads['W2'] = dw2 + self.reg * self.params['W2']
        grads['b2'] = db2

        # Calculate derivatives for first layer input, weight, bias
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # Initialise weights and biases for all fully connected layers with ReLU
        for i in range(self.num_layers - 1):
          out_dim = hidden_dims[i]
          self.params['W%d' %(i+1)] = np.random.normal(0, weight_scale, (in_dim, out_dim))
          ...
          in_dim = out_dim # Set input dim size of next layer to be output dim size of current layer

        # Initialise weights and biases for final FC layer
        out_dim = num_classes
        self.params['W%d' %(self.num_layers)] = np.random.normal(0, weight_scale, (in_dim, out_dim))
        self.params['b%d' %(self.num_layers)] = np.zeros(out_dim)
    ...
    def loss(self, X, y=None):
        ...
        # Do for all other FC layers
        for i in range(self.num_layers-1, 0, -1):
          if self.use_dropout:
            dhout = dropout_backward(dhout, dp_cache[i-1])
          if self.use_batchnorm:
            dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dhout, ar_cache[i-1])
            grads['gamma%d'%(i)] = dgamma
            grads['beta%d'%(i)] = dbeta
          else:
            # Calculate derivatives for input, weight, bias
            dx, dw, db = affine_relu_backward(dhout, ar_cache[i-1])   
